...
import com.kh.chap01_list.par01_arryList.model.vo.MusicAristDescending;
...
public class ListRun {
	 ...
	 * 	- 효율적인 기능들이 정의되어있는 틀
	 * 
	 * 컬렉션 한줄정리 => 데이터들이 새롭게 추가되거나 삭제가 되거나, 
	 * 수정이 되는 기능들을 이미 정의해놓은 틀
	 * 
	 * 배열과 컬렉션(List)의 차이점
	 *  - 배열의 단점
	 *  1. 같은자료형의 데이터만 저장가능
	 ...
	 *  	(제네릭 설정을 통해 한가지 유형의 데이터만 
	 *  	저장도 가능)
	 *  2. 중간에 값을 추가하거나, 삭제하는등 
	 *  	배열내부의 값들을 조작하기 위한 편리한 메서드가 
	 *  	정의 되어 있다.
	 *  3. 크기에 대한 제약이 없다.
	 *  	(배열깊은복사를 알아서 해준다.)
	 *  
	 ...
	 *   	(찾고자하는 값을 즉시 찾음 - 빠르다)
	 *  
	 *  컬레션 중 리스트와 셋계열은 기능적으로 연관이 있으나 
	 *  맵계열은 상대적으로 거리가 있다.
	 * */
	
	public static void main(String[] args) {
		// ArrayList list = new ArrayList();// 크기 10배열생성
		
		/*
		 * 
		 * 제네릭( <E> )
		 *  - 별도의 제네릭설정없이 컬렉션객체 생성시 
		 *    제네릭은 Object로 설정된다.
		 *  - 제네릭을 설정하면 설정한 타입의 값들만을 
		 *    컬렉션에 담을 수 있다.
		 *  
		 *  제네릭을 설정하는 이유
		 *  1. 명시한 타입의 객체만 저장가능하도록 하여 
		 *     "타입의 안정성"을 확보하기 위해서
		 *  2. 컬렉션에 저장된 객체를 꺼내서 사용할 때
		 *     직접 형변환하는 작업을 없애기 위해서
		 *     
		 * 
		 * */
		// 컬렉션에는 Music 타입의 객체만 저장 가능.
		ArrayList<Music>list = new ArrayList<>(3);
		//객체 생성시 제네릭 생략가능(java7이상, 타입추론기능)
		...
		// 중간에 값 추가시 알아서 기존값들을 뒤로 한칸식 땡켜주는 깊은 복사가 내부적으로 발생한다.
		System.out.println(list);
		...
		Comparator<Music> comp2 = new MusicAristDescending();
