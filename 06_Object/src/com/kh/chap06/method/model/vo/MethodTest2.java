...
public class MethodTest2 {
	 ...
	 * return;
	 * }
	 * - 메서드 호출시 객체를 생성하 필요가 없다.
	 * - 프로그램 시작시 정적메모리 영역에 메서드 정보가 저장되어 있기 떄문.
	 * - 클래스명, 메서드명(전달값);으로 호출하면 된다.
	 * 
	 * */
	
	static int count =1; // static, 프로그램 시작시
	...
	public static int method(int num1) {
		 ...
		 * - 변수의 생성시점이 다르기 때문
		 * - static메서드는 프로ㅛ그램 시작과 동시에 정적메모리영역에 할당되는데,
		 * 		같은 시점에 static변수 함꼐 할당되기 때문에 static변수는 사용 가능.
