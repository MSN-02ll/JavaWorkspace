...
public class A_Stringpool {
	...
	// - 문자열 데이터가 담기는 영역으로, Heap 메모리영역 상에 항상 고정적으로 할당되어 있다.
	public void method1() {
		String str1 = new String("hello");
		String str2 = new String("hello");
		
		System.out.println(str1==str2); // flase(bcs 주소값 비교)
		...
		// 변환해주는 메서드로 오버라이딩 되어있다.
		
		// 실제 참조변수의 주소값을 확인하는 메서드
		// System.identyHashCode(변수);
	 ...
	 * 3. 만약 이미 상수풀에 그 값이 저장되어 있다면, 저장되어잇는 주소값을 반환한다.(얕은 복사)
	 *    
	 * 
	 * */
		
	/* 
	 * hasCode메서드를 오버라이딩한 이유
	 * 	- 문자열은 문자열기반 hashCode값을 상수풀상의 주소값으로 사용하고 있다.
	 * 	  따라서 상수풀안에 데이터를 저장할 때 문자열을 해시코드값으로 변환한 후 
	 *    그 자리에 데이터가 있는지 없는지 검사를 실행하고, 이미 값이 있다면 
	 *    해당 주소값을 변수에 할당하거나 데이터가 없었다면 상수풀에 새롭게 값을 등록하고 
	 *    그 주소값을 반환한다.
	 * 
	 * */
		...
		System.out.println(str1 == str2); // false ==> 저장공간이 서로 다름
