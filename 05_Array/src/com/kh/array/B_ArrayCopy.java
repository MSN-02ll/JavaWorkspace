...
public class B_ArrayCopy {
	/*
	 * 배열복사
	 * - 얕은 복사 : 배열의 주소값만을 복사.//heap 메모리에 변화 X
	 * - 깊은 복사 : 동일한 새로운 배열을 하나 생성하여 
	 * 			  내부의 값들까지 복사하는 개념.
	 * 
	 * 
	 * 
	...
	public void method3() {
		// 2. 새로운 배열 생성 후 System클라스의 arraycopy를 이용한 복사.
		// c언어(성능)의 method를 사용 => 빠름.
		int [] origin = {1,2,3,4,5};
		int [] copy = new int [10];
		
		//System.arraycopy(원본배열, 원본배열의 복사를 시작할 인덱스, 복사본 배열, 
		//					복사본 배열의 복사를 시작할 인데스, 복사할 갯수)
		// System.arraycopy(원본, 복사 시작 위치, 붙여넣기할 대상, 붙여넣기 시작 위치, 개수(0);
		// System.arraycopy(origin, 0, copy, 0, 5);
		
		// API => 사용방법이 고정되어있는 method로 생각하면 좋다.
	...
	public void method4() {
		...
		int [] copy = Arrays.copyOf(origin, 10); 
			// 복사할 갯수 == 복사본 배열의 크기.
			// 복사할 갯수 > 원본 크기 = 나머지 자리 0으로 채워짐
			// 복사할 갯수 < 원본 크기 = 순차대로 입력됨(ex, 개수 3일때 [1,2,3]
